...
function navigateToPage(pageId) {
    const validPages = ['dashboard', 'data-mobil', 'kriteria', 'perhitungan-saw', 'perhitungan-wp', 'perhitungan-smart', 'ranking', 'visualisasi', 'tentang'];
...
function loadDashboardStats() {
    ...
    if (sawRes.ranking && sawRes.ranking.length > 0) {
        const topCar = sawRes.ranking[0];
        if (bestSawEl) bestSawEl.innerText = topCar.name;
        if (bestSawScoreEl) bestSawScoreEl.innerText = `Skor: ${topCar.score.toFixed(4)}`;
    } else {
        if (bestSawEl) bestSawEl.innerText = "-";
        if (bestSawScoreEl) bestSawScoreEl.innerText = "Skor: 0.0000";
    }
...
function renderWPTables() {
    ...
    if (tbodyS) {
        tbodyS.innerHTML = res.vectorS.map((c, idx) => `
            <tr>
                ...
                <td><strong>${c.s.toExponential(4)}</strong></td>
            </tr>
        `).join('');
    }
    ...
    if (sInfo) {
        sInfo.innerHTML = `<div class="p-3 border border-primary rounded bg-dark mt-3 text-center"><strong>Total Nilai Vektor S (&#931;S):</strong> ${res.sumS.toExponential(6)}</div>`;
    }
...
function renderRankingAndRecommendations() {
    ...
    // 1. RENDER TOP 5 ALTERNATIVE CAR CARDS
    ...
    if (cardsGrid) {
        if (sawRes.ranking.length === 0) {
            ...
            // Limit to top 5 cars by SAW score
                            ...
                            <div class="car-card-body">
                                <div class="car-specs">
                                    <div class="spec-item">
                                        ...
                                        <span class="spec-val">${car.engine.toLocaleString()} CC</span>
                                    </div>
                                    <div class="spec-item">
                                        <span class="spec-label">Kilometer</span>
                                        <span class="spec-val">${car.mileage.toLocaleString()} KM</span>
                                    </div>
                                    <div class="spec-item">
                                        <span class="spec-label">Kapasitas Kursi</span>
                                        <span class="spec-val">${car.seats} Penumpang</span>
                                    </div>
                                    <div class="spec-item">
                                        <span class="spec-label">Rank Lain (WP/SMART)</span>
                                        <span class="spec-val">Rank ${wpRank} / Rank ${smartRank}</span>
                                    </div>
                                </div>
